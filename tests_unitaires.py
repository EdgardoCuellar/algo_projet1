...
def test_8():
    ...
    try:
        ...
        print(
            "une excéption s'est produite dans l'ajout de la pièce Piece('b',matrice) à la postion [1,0] à un tableau qui contenait la piece Piece('a',matrice) à la position [0,0]," +
            "matrice = Ma_matrice([[0,1,0],[1,1,1],[0,1,0]])")

    if (tableau.tableau != bon_tableau):
        print('test 8 raté')
        print(
            "tableau.tableau a été modifié lors de la temptative d'ajout d'une pièce qui avait un overlap avec une autre. Input:")
        print("matrice = Ma_matrice([[0,1,0],[1,1,1],[0,1,0]])")
        print("liste_pieces = [Piece('a',matrice),Piece('b',matrice)]")
        print("tableau = Tableau([6,4], liste_pieces)")
        print("tableau.ajouter_piece(0,0,[0,0])")
    ...
    try:
        ...
        print(
            "une excéption s'est produite dans l'ajout de la pièce Piece('b',matrice) à la postion [3,0] à un tableau qui contenait la piece Piece('a',matrice) à la position [0,0]," +
            "matrice = Ma_matrice([[0,1,0],[1,1,1],[0,1,0]])")
...
def test_11():
    ...
    if (len(solution) != 3):
        print('test 11 raté')
        print(
            "lorsqu'on lance, solution = possibles_factorisations(16), solution devrait avoir 3 éléments: (1,16), (2,8) et (4,4). L'ordre n'est pas important. Vous n'avez pas 3 exactement factorisations.")
    elif ((1, 16) not in solution and (16, 1) not in solution):
        print('test 11 raté')
        print(
            "lorsqu'on lance: solution = possibles_factorisations(16), (1,16) ou (16,1) doit être contenu dans la solution et ce n'est pas le cas avec votre code")
    elif ((2, 8) not in solution and (8, 2) not in solution):
        print('test 11 raté')
        print(
            "lorsqu'on lance: solution = possibles_factorisations(16), (2,8) ou (8,2) doit être contenu dans la solution et ce n'est pas le cas avec votre code")
    elif ((4, 4) not in solution):
        print('test 11 raté')
        print(
            "lorsqu'on lance: solution = possibles_factorisations(16), (4,4) doit être contenu dans la solution et ce n'est pas le cas avec votre code")
    else:
...
def test_13():
    ...
    for tableau in liste_solutions:
        tableau.imprimer()
    print(
        "Les tableaux doivent avoir les mêmes dimensions que ceux donnés, toutes les pièces doivent être présentes et la hauteur du premier tableau doit être plus petite que celle du deuxième tableau")
